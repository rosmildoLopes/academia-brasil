	...
	return (
		<div className='relative w-full pt-4'>
			...
			<Image
				...
				width={116}
				height={130}
				className='absolute w-[63px] h-[47px] z-10 top-24 left-10 md:w-[116px] md:h-[130px] md:left-[240px] md:top-24'
			/>
			...
			<Image
				...
				width={144}
				height={183}
				className='absolute w-[82px] h-[58px] z-50 top-24 right-6 md:w-[144px] md:h-[183px] md:right-40 md:rotate-3'
			/>
			<div className='flex flex-col pt-4 w-[70%] space-y-3 mx-auto font-black text-xs text-firstBlue md:flex-row md:space-x-5 md:w-full md:space-y-0 md:justify-center'>
				<button onClick={handleButtonClick} className='px-3 py-2 border-2 border-firstBlue text-center rounded-3xl hover:bg-firstBlue hover:text-black md:text-lg md:px-16 md:py-3 md:text-black md:bg-firstBlue md:rounded-full md:hover:bg-transparent md:hover:text-firstBlue'>
					Descargar Programa
				</button>
				<HablarConAsesor
					message='Estoy interesado en el curso de Wordpress , me gustaría recibir mas información'
					phoneNumber='5491131262388'
				/>
				<Link
					href='https://tienda.academianumen.com.ar/producto/desarrollo-wordpress/'
					target='_blank'
			...
			{showModal && <ProgramaFullstack onClose={() => setShowModal(false)}/>}
		</div>
	);
}
